...
(function(ua, doc, undefined) {
	...
	Obj.prototype = {
		...
		*/		 
			...
			setInterval(function(){
				...
				$("img[data-src]:visible").each(function() {
					...
					var _this = $(this),
						...
						errpic =  opt.error || 'images/error.gif'; //错误图片地址        
					...
					if (ot < st + ch && ot + oh > st) {
						...
						_this.error(function() {
							if (new RegExp(errpic).test(_this.attr('style'))) return false;
		...
		cookie: function(name, value, op) {
			...
			if (typeof value != 'undefined') {
					...
					if (op.expires && (typeof op.expires == "number" || op.expires.toUTCString)) {
							if (typeof op.expires == "number") d.setTime(d.getTime() + (op.expires * 24 * 60 * 60 * 1000))
							else d = op.expires;
		...
		/**
		 * loadjs 同步或异步方式加载js文件 op:
		 * @param  {[type]} src 加载js地址
		 * @param  {[type]} op  {charset,id,data,async:true} 默认同步输出
		 */
		loadjs: function(src, op) {
			var op = op || {};
			if (op.async) {
				...
				if (op.charset) obj.charset = op.charset;
				if (op.data) obj.data = op.data;
				if (op.id) obj.id = op.id;
				(doc.head || doc.getElementsByTagName('head')[0] || doc.docElement).appendChild(obj);
			} else doc.write('<script src="' + src + '"' + (op.charset ? ' charset="' + op.charset + '"' : '') + (op.data ? ' data="' + op.data + '"' : '') + (op.id ? ' id="' + op.id + '"' : '') + '></script>');
		},
		...
		floatAD: function(op) {
			...
			if (this.isMobile()) {
				arrAd = op.mobile || [];
				if (/(iPhone|iPad|iPod)/ig.test(ua)) //iOS单独处理
					arrAd = [];
			} else {
				// PC版广告展示对联和富媒体数据间隔排列
				arrAd = op.pc || [];
		 ...
		 * }
		 */
		taobaoAD: function(op) {
			var id = op.id,
				url = op.url || "http://p.tanx.com/ex?i=mm_"
			charset = op.charset || "utf-8";
		...
		baiduAD: function(op) {
			var id = op.id,
				url = op.url || 'http://cpro.baidustatic.com/cpro/ui/',
				flag = op.flag || 'c',
				api = op.api || {};
		 ...
		 * }
		 */
		inlay360AD: function(op) {
			var api = op.api,
				url = op.url || "http://s.lianmeng.360.cn/so/inlay.js"
			charset = op.charset || "utf-8";
	...
	//返回全局对象
	maigouwang = window.maigouwang = new Obj();
...
function modifyFn(obj){
	...
	$(window).resize(function(){
		left = curMenu.position().left + offset;
		modifyObj.animate({left:left,width:curMenu.width()}, 100);left;
	})
	modifyObj.animate({left:left,width:curMenu.width()}, 100);left
}
...
tabs.each(function(){
	...
	_this
		.on('mouseover','li', function(e) {
			...
			_this.parents('.tabs').find('.bd .tabs-item').eq(index).show().siblings('.tabs-item').hide();
		})
